-- Teacher Advertisement tab parity with the student "Post an ad" composer
-- (studentDashboard.wantedAds): a batch now carries the same medium/
-- class_type facts a wanted ad already does, settable from the teacher's ad
-- form (upsertBatchAd/createIndividualAd) alongside the subject/rate fields
-- it already sets there. Nullable, no default -- an existing batch simply
-- shows no medium/class-type chip until its ad is next saved.

alter table batches add column if not exists medium text check (medium in ('english', 'sinhala', 'tamil', 'other'));
alter table batches add column if not exists class_type text check (class_type in ('new', 'revision'));

-- Adding columns to a function's return table changes its row type, which
-- create-or-replace refuses ("cannot change return type of existing
-- function") -- drop first, same as any other migration that widens an
-- existing RPC's Returns table.
