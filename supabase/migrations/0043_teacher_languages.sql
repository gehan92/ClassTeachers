-- Languages a teacher teaches in (e.g. English, Sinhala, Tamil) — same
-- shape as qualifications/work_experience (0004): a plain text[] column,
-- edited as a comma-separated list like subjects rather than a repeatable
-- row UI, since entries here are single words, not sentences.
alter table teacher_profiles add column if not exists languages text[];

-- Return shape gained a column, which create-or-replace can't do for a
-- `returns table` function — has to be dropped first (same as 0042).
