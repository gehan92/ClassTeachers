-- Optional price range for a teacher's per-class/individual ad rate (0119's
-- Advertisement tab work) -- hourly_rate/monthly_rate stay the "starting"
-- rate exactly as before; these two new columns are only ever set alongside
-- them, never alone, and only from the teacher's ad form. Profile default
-- rates and institute batch rates are untouched -- still a single number.

alter table batches add column if not exists hourly_rate_max numeric(10, 2);
alter table batches add column if not exists monthly_rate_max numeric(10, 2);

-- Same "cannot change return type of existing function" restriction hit by
-- 0119 -- drop before recreating with the two new output columns.
...
as $$
  select
    ...
    b.medium,
    b.class_type,
    -- Only meaningful when the batch itself set a hourly/monthly rate (the
    -- range is per-batch, never inherited from the profile default like the
    -- base rate above is) -- a batch relying on the profile default has no
    -- range regardless of what this column holds.
